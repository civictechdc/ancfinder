...
from forms import ANCUpdateForm, SMDUpdateForm
...
class ANCUpdateView(FormView):
    template_name = 'ancfindersite/anc-update-form.html'
    ...
    def form_valid(self, form):
        ...
        form.send_email()
        return super(ANCUpdateView, self).form_valid(form)


class SMDUpdateView(FormView):
    template_name = 'ancfindersite/smd-update-form.html'
    ...
    def form_valid(self, form):
        ...
        # It should return an HttpResponse.
        form.send_email()
        return super(SMDUpdateView, self).form_valid(form)


# assemble census data on first load
def make_anc_hex_color(anc):
    def avg(t1, f1, t2, f2):
        return (int((t1[0]*f1+t2[0]*f2)/(f1+f2)),
                int((t1[1]*f1+t2[1]*f2)/(f1+f2)),
                int((t1[2]*f1+t2[2]*f2)/(f1+f2)))

    def hexish(tup):
        return "".join(("%0.2X" % d) for d in tup)

    ward_color_set = [
        (27, 158, 119), (217, 95, 2), (166, 118, 29), (117, 112, 179),
        (231, 41, 138), (102, 166, 30), (230, 171, 2), (166, 118, 29)
    ]

    anc_color_set = [
        (228, 26, 28), (55, 126, 184), (77, 175, 74), (152, 78, 163),
        (255, 127, 0), (255, 127, 0), (166, 86, 40)
    ]
...
for ward in anc_data.values():
    for anc in ward["ancs"].values():
        for key, info in anc["census"].items():
            census_grids.setdefault(key, []).append(
                (anc["anc"], info["value"], make_anc_hex_color(anc["anc"])))
...
class AncInfoTemplateView(TemplateView):
    ...
    def get(self, request, anc, *args, **kwargs):
        ...
        for m in all_meetings:
            if m > now:
                # this is the first meeting in the future
                # (i.e. the next meeting)
                next_meeting = m
                break
        ...
        for mtg in previous_meetings + ([next_meeting] if next_meeting else []):
            ...
            for doc_type_id, doc_type_name in [(14, "Summary"), (2, "Minutes"), (1, "Agenda")]:
                # If minutes or a summary exist for a meeting, don't bother
                # displaying an agenda (or ask to upload an agenda) for the
                # meeting.
                if has_doc and doc_type_id == 1:
                    continue

                # in case there are two documents of the same type, just get
                # the first
                def first(qs):
                    if qs.count() == 0:
                ...
                # find the document
                try:
                    doc = first(Document.objects.filter(
                        anc=anc, doc_type=doc_type_id, meeting_date=mtg))
                    has_doc = True
                except Document.DoesNotExist:
                    doc = None

                # for meetings that aren't behind us, if a summary isn't
                # available don't bother asking the user to upload one.
                if not doc and doc_type_id == 14 and mtg >= now:
                    continue

                # for ANCs that have never had a summary posted, don't ask for
                # one either
                if not doc and doc_type_id == 14 and not Document.objects.filter(anc=anc, doc_type=doc_type_id).exists():
                    continue

                # for meetings that aren't two weeks behind us, if minutes
                # aren't available, don't bother asking for them because they
                # are almost certainly not available yet
...
def prep_hoods(info, is_anc):
    def is_part(h):
        if h.get("part-of-neighborhood", 0) < (.9 if is_anc else .6):
            return "parts of "
        return ""
...
class DocumentTemplateView(TemplateView):
    ...
    def get(self, request, anc=None, date=None, id=None, slug=None, *args,
            **kwagrs):
